...
Output: [[1,7]]
Explanation: Intervals [1,4] and [4,7] are considere
...
function mergeInterval(interval){
  ...
  for(let i = 1; i < interval.length; i++){
    if(interval[i][0] <= ans[ans.length - 1][1]){
        ans[ans.length - 1][1] = Math.max(ans[ans.length-1][1], interval[i][1]);
    }else{
        ans.push(interval[i])
    }
  }
